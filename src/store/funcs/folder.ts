...
function systemMsg(name: string): Message {
  return {
    role: "system",
    content: `Folder "${name}" is ready. Always call list_directory('.') first to see what's inside, then read files as needed. For images (PNG, JPEG, GIF, etc.), use describe_image instead of read_file. If the user asks a general question without specifying a file, explore the folder yourself to find relevant files.`,
  };
}
